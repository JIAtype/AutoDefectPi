...
def main():
    ...
    # PORT = 3389
    # RASPBERRY_PI_IP = "0.0.0.0"  # 修改为你的实际IP
    PORT = 4840    
    ...
    try:
        ...
        # 在 client.py 的 main 函数中，步骤6
        # ...
        for child in camera_children:
            browse_name_obj = child.get_browse_name() # Это QualifiedName
            # 构建 "ns:Name" 格式的字符串键
            browse_name_key = f"{browse_name_obj.NamespaceIndex}:{browse_name_obj.Name}"
            node_class = child.get_node_class()

            try:
                if node_class == ua.NodeClass.Method: # 使用 ua.NodeClass 枚举更精确
                    methods[browse_name_key] = child
                    print(f"  📋 方法: {browse_name_key}")
                elif node_class == ua.NodeClass.Variable: # 使用 ua.NodeClass 枚举更精确
                    variables[browse_name_key] = child
                    value = child.get_value()
                    print(f"  📊 变量: {browse_name_key} = {value}")
            except Exception as e:
                print(f"  ⚠️ 读取 {browse_name_key} 时出错: {e}")

        # 在 client.py 的 main 函数中
        # ... （步骤6 之后） ...
        print("步骤7: 测试功能...")

        # 从 camera_object 获取正确的命名空间索引，用于构造方法/变量的 QualifiedName 字符串
        cam_obj_ns_idx = camera_object.nodeid.NamespaceIndex # 通常是 2，根据你的服务器设置

        # 测试单张拍照
        capture_image_method_name_str = f"{cam_obj_ns_idx}:CaptureImage" # e.g., "2:CaptureImage"
        if capture_image_method_name_str in methods:
            capture_method_node = methods[capture_image_method_name_str] # 获取方法节点对象
            print(f"\n测试拍照方法: {capture_image_method_name_str} (Node: {capture_method_node})")
            try:
                # 调用方法：第一个参数是方法节点本身 (或者它的NodeId)
                # 第二个及后续参数是方法的输入参数 (你的方法没有输入参数)
                result_variant_list = camera_object.call_method(capture_method_node)
                # OPC UA方法调用返回一个包含输出参数Variant的列表
                if result_variant_list and isinstance(result_variant_list, list):
                    print(f"✅ 拍照结果: {result_variant_list[0]}") # 服务器端返回一个字符串
                else:
                    print(f"✅ 拍照结果 (原始): {result_variant_list}")

            except Exception as e:
                print(f"❌ 拍照失败: {e}")
        else:
            print(f"⚠️ 未找到拍照方法 '{capture_image_method_name_str}'. 可用方法: {list(methods.keys())}")

        # 测试运动检测 (类似地修改)
        start_motion_method_name_str = f"{cam_obj_ns_idx}:StartMotionDetection"
        stop_motion_method_name_str = f"{cam_obj_ns_idx}:StopMotionDetection"

        if start_motion_method_name_str in methods and stop_motion_method_name_str in methods:
            start_method_node = methods[start_motion_method_name_str]
            stop_method_node = methods[stop_motion_method_name_str]

            print(f"\n测试运动检测...")
            try:
                print(f"调用: {start_motion_method_name_str}")
                result = camera_object.call_method(start_method_node)
                if result and isinstance(result, list): print(f"启动运动检测: {result[0]}")
                else: print(f"启动运动检测 (原始): {result}")

                time.sleep(3) # 等待几秒

                print(f"调用: {stop_motion_method_name_str}")
                result = camera_object.call_method(stop_method_node)
                if result and isinstance(result, list): print(f"停止运动检测: {result[0]}")
                else: print(f"停止运动检测 (原始): {result}")

            except Exception as e:
                print(f"❌ 运动检测测试失败: {e}")
        else:
            if not start_motion_method_name_str in methods:
                print(f"⚠️ 未找到启动运动检测方法 '{start_motion_method_name_str}'. 可用: {list(methods.keys())}")
            if not stop_motion_method_name_str in methods:
                print(f"⚠️ 未找到停止运动检测方法 '{stop_motion_method_name_str}'. 可用: {list(methods.keys())}")
